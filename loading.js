...
async function fetchSubmissions() {
    try {
        const { data, error } = await supabase
            .from('submission')
            .select('*, media:media_id(*), hero_media:hero_media_id(*), video_media:video_media_id(*)')
        ...
        data.forEach(submission => {
            ...
            if (submission.video_media) {
                const videoUrl = supabase.storage.from(submission.video_media.bucket).getPublicUrl(submission.video_media.file_path);
                allContentData.push({
                    type: 'video',
                    url: videoUrl.data.publicUrl,
                    label: submission.fish_type,
                    source: 'submission'
                });
            }
